...
$query_last_number = "SELECT MAX(SUBSTRING(id_form_item, LENGTH(?) + 1)) as last_number FROM tb_form_item WHERE id_pengecekan = ?";
$stmt_last_number = mysqli_prepare($conn_pm, $query_last_number);
mysqli_stmt_bind_param($stmt_last_number, "ss", $id_pengecekan, $id_pengecekan);
...
$id_form_item = $id_pengecekan . $new_number;

// Query untuk menyimpan data ke dalam tabel form_item
$query_insert = "INSERT INTO tb_form_item (id_form_item, id_pengecekan, id_form, id_item) VALUES (?, ?, ?, ?)";
...
foreach ($id_item_values as $id_item) {
    ...
    mysqli_stmt_bind_param($stmt, "ssss", $id_form_item, $id_pengecekan, $id_form, $id_item);
    ...
    $new_number++;
    $id_form_item = $id_pengecekan . sprintf('%07d', $new_number);
}
